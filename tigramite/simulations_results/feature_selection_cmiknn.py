...
# Print the DataFrame
print(df_1)
df_1.to_csv('/home/gnicolaou/tigramite/tigramite/simulations_results/feature_selection_cmiknn_5_extended.csv')
...
# Print the DataFrame
print(df_2)
df_2.to_csv('/home/gnicolaou/tigramite/tigramite/simulations_results/feature_selection_cmiknn_10_extended.csv')
...
# Print the DataFrame
print(df_3)
df_3.to_csv('/home/gnicolaou/tigramite/tigramite/simulations_results/feature_selection_cmiknn_15_extended.csv')
...
def generate_dataset(n, nan_ratio=0.1, phi_latent1=0.8, phi_latent2=0.7):
    # Initialize latent confounders
    latent1 = np.zeros(n)
    latent2 = np.zeros(n)
...
result = feature_selection_lpcmci(data)
elapsed_time.append(result[1])
f1.append(result[2])
cmi.append(result[3])
recall.append(result[4])
...
def generate_dataset(n, nan_ratio, phi=0.8):
    ...
    # Generate y6 influenced by latent and non-linear transformations of y5
    y6[0] = np.random.normal()
    for t in range(1, n):
        y6[t] = 0.6 * latent[t] + 0.4 * np.sin(y5[t]) + np.random.normal()

    # Generate y7 influenced by y3 and lagged y6
    y7[0] = np.random.normal()
    for t in range(1, n):
        y7[t] = 0.7 * np.square(y3[t]) + 0.6 * np.roll(y6, 1)[t] + np.random.normal()

    # Generate y8 influenced by latent and y7
    y8[0] = np.random.normal()
    for t in range(1, n):
        y8[t] = 0.8 * latent[t] + 0.5 * np.sqrt(np.abs(y7[t])) + np.random.normal()

    # Generate y9 influenced by y6 and y8
    y9[0] = np.random.normal()
    for t in range(1, n):
        y9[t] = 0.4 * np.log(np.abs(y6[t]) + 1) + 0.7 * y8[t] + np.random.normal()

    # Generate y10 influenced by latent and y9
    y10[0] = np.random.normal()
    for t in range(1, n):
        y10[t] = 0.6 * latent[t] + 0.5 * y9[t] + np.random.normal()
    ...
    # Ground truth causal matrix with lagged and contemporaneous causal relationships
    ground_truth = np.array([
        [['', '-->'], ['<--', ''], ['', ''], ['-->','<--'], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['<--', ''], ['', ''], ['o-o', ''], ['-->', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['', ''], ['o-o', ''], ['', ''], ['-->', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
...
def generate_dataset(n, nan_ratio, phi=0.8):
    ...
    # Initialize variables y1 to y15
    variables = {f'y{i}': np.zeros(n) for i in range(1, 16)}

    # Generate y1 using AR(1) with non-linear transformation
    variables['y1'][0] = np.random.normal()
    for t in range(1, n):
        variables['y1'][t] = 0.9 * variables['y1'][t-1] + np.random.normal()
    variables['y1'] = np.log(np.abs(variables['y1']) + 1) + np.random.normal(scale=0.1, size=n)

    # Generate causal influences with non-linear transformations for other variables
    for t in range(1, n):
        variables['y2'][t] = 0.5 * latent[t] + 0.3 * np.tanh(variables['y1'][t]) + np.random.normal()
        variables['y3'][t] = 0.6 * latent[t] + 0.8 * np.square(variables['y2'][t]) + np.random.normal()
        variables['y4'][t] = 0.4 * variables['y1'][t-1] + 0.9 * np.sqrt(np.abs(variables['y3'][t])) + np.random.normal()
        variables['y5'][t] = 0.7 * latent[t] + 0.5 * np.log(np.abs(variables['y2'][t]) + 1) + 0.6 * variables['y4'][t-1] + np.random.normal()
        variables['y6'][t] = 0.6 * latent[t] + 0.4 * np.sin(variables['y5'][t]) + np.random.normal()
        variables['y7'][t] = 0.7 * np.square(variables['y3'][t]) + 0.6 * variables['y6'][t-1] + np.random.normal()
        variables['y8'][t] = 0.8 * latent[t] + 0.5 * np.sqrt(np.abs(variables['y7'][t])) + np.random.normal()
        variables['y9'][t] = 0.4 * np.log(np.abs(variables['y6'][t]) + 1) + 0.7 * variables['y8'][t] + np.random.normal()
        variables['y10'][t] = 0.6 * latent[t] + 0.5 * variables['y9'][t] + np.random.normal()
        variables['y11'][t] = 0.5 * np.tanh(variables['y10'][t]) + 0.4 * np.square(variables['y5'][t]) + np.random.normal()
        variables['y12'][t] = 0.3 * np.sin(variables['y7'][t]) + 0.6 * variables['y11'][t] + np.random.normal()
        variables['y13'][t] = 0.7 * np.log(np.abs(variables['y8'][t]) + 1) + 0.5 * variables['y12'][t-1] + np.random.normal()
        variables['y14'][t] = 0.8 * latent[t] + 0.3 * np.sqrt(np.abs(variables['y13'][t])) + np.random.normal()
        variables['y15'][t] = 0.6 * np.square(variables['y14'][t]) + 0.4 * variables['y9'][t-1] + np.random.normal()

    # Introduce NaN values randomly
    nan_indices = np.random.choice(n, size=int(n * nan_ratio), replace=False)
    for key in variables:
        variables[key][nan_indices] = np.nan

    # Combine into DataFrame
    data = pd.DataFrame(variables)

    # Ground truth causal matrix accurately reflecting the causal graph
    ground_truth = np.array([
        [['', '-->'], ['', ''], ['', ''], ['', '-->'], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['', ''], ['', ''], ['', ''], ['', ''], ['o-o', '-->'], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['', ''], ['o-o', ''], ['', ''], ['', '-->'], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
...
def generate_dataset(n, nan_ratio, phi=0.8):
    ...
    # Initialize variables y1 to y20
    variables = {f'y{i}': np.zeros(n) for i in range(1, 21)}
    ...
    for t in range(1, n):
        ...
        variables['y14'][t] = 0.8 * latent[t] + 0.3 * np.sqrt(np.abs(variables['y13'][t])) + np.random.normal()
        variables['y15'][t] = 0.6 * np.square(variables['y14'][t]) + 0.4 * variables['y9'][t-1] + np.random.normal()
        variables['y16'][t] = 0.7 * latent[t] + 0.6 * variables['y10'][t-1] + 0.5 * np.log(np.abs(variables['y15'][t]) + 1) + np.random.normal()
        variables['y17'][t] = 0.6 * variables['y11'][t] + 0.5 * np.square(variables['y16'][t]) + np.random.normal()
        variables['y18'][t] = 0.5 * np.tanh(variables['y17'][t]) + 0.4 * np.sqrt(np.abs(variables['y12'][t])) + np.random.normal()
        variables['y19'][t] = 0.7 * latent[t] + 0.6 * variables['y18'][t-1] + np.random.normal()
        variables['y20'][t] = 0.5 * np.square(variables['y19'][t]) + 0.4 * np.sin(variables['y13'][t]) + np.random.normal()
    ...
    # Ground truth causal matrix with accurate relationships for 20 variables
    ground_truth = np.array([
        [['', '-->'], ['<--', ''], ['', ''], ['-->',''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['', ''], ['', ''], ['o-o', ''], ['-->', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['', ''], ['o-o', ''], ['', ''], ['-->', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
...
def generate_dataset(n, nan_ratio, phi=0.8):
    ...
    # Initialize variables y1 to y25
    variables = {f'y{i}': np.zeros(n) for i in range(1, 26)}
    ...
    for t in range(1, n):
        ...
        variables['y19'][t] = 0.7 * latent[t] + 0.6 * variables['y18'][t-1] + np.random.normal()
        variables['y20'][t] = 0.5 * np.square(variables['y19'][t]) + 0.4 * np.sin(variables['y13'][t]) + np.random.normal()
        variables['y21'][t] = 0.6 * np.tanh(variables['y20'][t]) + 0.5 * latent[t] + np.random.normal()
        variables['y22'][t] = 0.4 * np.square(variables['y15'][t]) + 0.6 * np.log(np.abs(variables['y21'][t]) + 1) + np.random.normal()
        variables['y23'][t] = 0.5 * latent[t] + 0.3 * np.sqrt(np.abs(variables['y22'][t])) + np.random.normal()
        variables['y24'][t] = 0.7 * np.sin(variables['y23'][t]) + 0.4 * variables['y18'][t-1] + np.random.normal()
        variables['y25'][t] = 0.6 * np.square(variables['y24'][t]) + 0.5 * variables['y16'][t-1] + np.random.normal()
    ...
    # Ground truth causal matrix with accurate relationships for 25 variables
    ground_truth = np.array([
        [['', '-->'], ['<--', ''], ['', ''], ['-->',''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['', ''], ['', ''], ['o-o', ''], ['-->', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
        [['', ''], ['o-o', ''], ['', ''], ['-->', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', ''], ['', '']],
